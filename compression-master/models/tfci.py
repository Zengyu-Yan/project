...
def compress_image(model, input_image):
  ...
  with tf.Graph().as_default():
    ...
    # np.savez('foo.npz', arrays[1], arrays[2])
    np.savez('foo.npz',packed.string)
    #print(arrays)
    #print(packed.string)
...
def main(args):
  ...
  if args.command == "compress":
      ...
      if os.path.isdir(input_file):
          print("ISDIRECTORY")
          # get all the files in the directory 'input_file'
          # if input_file/f is not a file, e.g. if it is a directory, it will not be added
          files_in_directory = [f for f in listdir(input_file) if isfile(join(input_file, f))]
          print(files_in_directory)
          print(input_file)
          for file in files_in_directory:
              if file.split(".")[-1] == 'png':
                  file_path = join(input_file,file)
                  # file_path = input_file + "/" + file
                  # print(file_path)
                  compress(args.model, file_path,
                            args.output_file, args.target_bpp, args.bpp_strict)
          files_in_directory.sort()
          for file in files_in_directory:
              if file.split(".")[-1] == 'png':
                  file_path = join(input_file, file)
                  print(file_path + " " + str(Path(file_path).stat().st_size) + " " + str(Path(file_path+".tfci").stat().st_size))
  ...
  if args.command == "decompress":
      ...
      if os.path.isdir(input_file):
          files_in_directory = [f for f in listdir(input_file) if isfile(join(input_file, f))]
          for file in files_in_directory:
              if file.split(".")[-1] == 'tfci':
                  file_path = join(input_file,file)
                  decompress(file_path, args.output_file)
          files_in_directory.sort()
          with open('size.csv','w') as file:
              writer = csv.writer(file)
              writer.writerow(['Compressed','Decompressed'])
              for file in files_in_directory:
                  if file.split(".")[-1] == 'tfci':
                      file_path = join(input_file, file)
                      print(file_path + " " + str(Path(file_path).stat().st_size) + " " + str(
                          Path(file_path + ".png").stat().st_size))
                      writer.writerow([str(Path(file_path).stat().st_size),str(Path(file_path+".png").stat().st_size)])
